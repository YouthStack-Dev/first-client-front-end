...
function Avatar({ name, role, id, variant = "emp" }) {
  return (
    <span
      style={variant === "emp" ? styles.avatarEmp : styles.avatarDrv}
      title={displayName(name, role, id)}
    >
  ...
  return (
    <div style={styles.statsRow}>
      {cards.map((c) => (
        <div
          key={c.label}
          style={{ ...styles.statCard, borderTop: `3px solid ${c.color}` }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            <div>
              ...
              <div style={{ ...styles.statValue, color: c.color }}>
                {c.value}
              </div>
            </div>
  ...
  return (
    <div style={styles.paginationBar}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {total > 0 && (
          <span style={{ ...styles.pgInfo, marginLeft: 10, color: "#444" }}>
      ...
      <div style={styles.pgBtns}>
        <button
          style={{
            ...styles.pgBtn,
            ...(page <= 1 || loading ? styles.pgBtnDisabled : {}),
          }}
        ...
        {getPageNumbers().map((p, i) =>
          p === "…" ? (
            <span key={`e${i}`} style={styles.pgEllipsis}>
              …
            </span>
          ) : (
            <button
              ...
              style={{
                ...styles.pgBtn,
                ...(p === page ? styles.pgBtnActive : {}),
                ...(loading ? styles.pgBtnDisabled : {}),
              }}
              disabled={loading}
        ...
        <button
          style={{
            ...styles.pgBtn,
            ...(page >= totalPages || loading ? styles.pgBtnDisabled : {}),
          }}
...
function TranscriptModal({ session: s, onClose }) {
  ...
  const transcript = useSelector(
    (state) =>
      state.chat.transcripts[s.booking_id] ?? {
        messages: [],
        total: 0,
        page: 1,
        per_page: 50,
      ...
      <div style={styles.modalBox}>
        ...
        <div style={styles.participantsBar}>
          <div style={styles.participantChip}>
            ...
            <div>
              ...
              <div style={styles.chipRole}>
                Employee · {LANG_NAMES[empLang] ?? empLang}
              </div>
            </div>
          </div>
          ...
          <div style={styles.participantChip}>
            ...
            <div>
              ...
              <div style={styles.chipRole}>
                Driver · {LANG_NAMES[drvLang] ?? drvLang}
              </div>
            </div>
          </div>
        ...
        {error && (
          <div style={{ padding: "0 20px 10px" }}>
            <ErrorBanner message={error} onRetry={() => goToPage(page)} />
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div style={styles.modalFooter}>
            <span style={styles.pgInfo}>
              {(page - 1) * per_page + 1}–{Math.min(page * per_page, total)} of{" "}
              {total} messages
            </span>
            <div style={styles.pgBtns}>
              <button
      ...
      <div style={{ maxWidth: "62%" }}>
        <div
          style={{
            ...
            color: "#aaa",
            marginBottom: 3,
            textAlign: isEmployee ? "left" : "right",
          }}
        >
          {senderLabel}
        </div>
        <div style={isEmployee ? styles.bubbleEmp : styles.bubbleDrv}>
          {msg.original_text}
        </div>
        {hasTranslation && (
...
export default function ChatSessionsTable() {
  ...
  useEffect(() => {
    dispatch(
      fetchChatSessions({ skip: (page - 1) * pageSize, limit: pageSize }),
    );
...
function SessionRow({ session: s, search, idx, onView }) {
  return (
    <tr
      style={{ ...styles.row, background: idx % 2 === 0 ? "#fff" : "#fafbfc" }}
    >
