...
const TARGET_TYPES_WITH_IDS = new Set([
  "specific_employees",
  "specific_drivers",
  "teams",
  "vendor_drivers",
...
const AnnouncementForm = ({ announcement, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (
      DRIVER_TARGET_TYPES.has(announcement.target_type) &&
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const payload = {
      ...
      channels,
      ...(TARGET_TYPES_WITH_IDS.has(formData.target_type) && {
    ...
    switch (formData.target_type) {
      case "specific_employees":
        return employees.map((e) => ({
          ...
          label: e.name ?? e.full_name ?? `Employee ${e.employee_id ?? e.id}`,
        }));
      case "specific_drivers":
      case "vendor_drivers":
        return drivers.map((d) => ({
          id: d.driver_id ?? d.id,
          label: d.name ?? `Driver ${d.driver_id ?? d.id}`,
        }));
      case "teams":
        return teams.map((t) => ({
          id: t.team_id ?? t.id,
          label: t.name ?? `Team ${t.team_id ?? t.id}`,
        }));
      default:
        return [];
    }
  ...
  return (
    ...
    >
      {apiError && (
        <div
          role="alert"
          className="mx-1 mb-3 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2"
        >
          <AlertCircle
            size={14}
          ...
          <div className="flex flex-wrap gap-1.5">
            {CONTENT_OPTS.map(({ value, label, Icon }) => (
              <button
                ...
                disabled={isSubmitting}
                onClick={() =>
                  setFormData((p) => ({ ...p, content_type: value }))
                }
            ...
            <div className="mt-2">
              ...
              {formData.media_file ? (
                <div className="flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2">
                  ...
                  {(() => {
                    const FIcon = FILE_ICON[formData.content_type];
                    return (
                      <FIcon
                        size={16}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-400">
                      {fmtBytes(formData.media_file.size)}
                    </p>
                  </div>
                  <label
                    htmlFor="af-file"
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-semibold text-gray-700">
                      Upload {formData.content_type}
                    </p>
                    <p className="text-[11px] text-gray-400">
                      {FILE_EXT_HINT[formData.content_type]}
                    </p>
                  </div>
                </label>
              )}
        ...
        <div>
            ...
            {TARGET_OPTS.map((opt) => {
              ...
              return (
                <button
                  ...
                  disabled={isSubmitting}
                  onClick={() =>
                    setFormData((p) => ({ ...p, target_type: opt.value }))
                  }
          ...
          {showTargetList && needsVendor && (
            <div className="mt-2">
              {hasEditIds && (
                <div className="mb-1.5 flex items-center justify-between rounded-lg bg-indigo-50 border border-indigo-200 px-3 py-1.5">
                  <span className="text-[11px] text-indigo-700 font-medium">
                    ✓ {editModeIds.length} driver
                    {editModeIds.length !== 1 ? "s" : ""} selected
                  </span>
                  <span className="text-[11px] text-indigo-500">
                    Pick vendor to change
                  </span>
                </div>
              )}
              <select
              ...
              >
                <option value="">
                  {hasEditIds ? "— Keep current —" : "— Choose a vendor —"}
                </option>
                {vendorOpts.map((v) => (
              ...
              {hasEditIds && !selectedVendorId ? (
                <div className="flex flex-wrap gap-1.5 p-2 border border-gray-200 rounded-lg bg-white">
                  {editModeIds.map((id) => (
                    <span
                      key={id}
                      className="px-2 py-0.5 rounded-lg bg-indigo-100 text-indigo-700 text-[11px] font-semibold border border-indigo-200"
                    >
                      #{id}
                    </span>
                  ))}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {Object.entries(CHANNEL_CFG).map(([ch, cfg]) => {
              ...
              return (
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${active ? "bg-current" : "bg-gray-300"}`}
                  />
                  {cfg.label}
                  {locked && (
