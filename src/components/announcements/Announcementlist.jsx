        ...
        <div className="flex flex-col flex-1 px-4 pt-3.5 pb-3 gap-2.5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium ${status.pill}`}
              >
                <span
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {item.total_recipients > 0 && (
              <span className="ml-auto inline-flex items-center gap-1 text-[11px] text-gray-400">
                <Icons.Users />
                {item.total_recipients} recipient
                {item.total_recipients !== 1 ? "s" : ""}
              </span>
            )}
          </div>
  ...
  const handleEdit = useCallback(
    async (item) => {
      ...
      const result = await dispatch(
        fetchAnnouncementById(item.announcement_id),
      );
      setEditingId(null);
  ...
  return (
    <div>
      ...
      {publishError && (
        <div
          role="alert"
          className="mb-4 flex items-center justify-between rounded-xl border border-orange-200 bg-orange-50 px-4 py-2.5 text-sm text-orange-700"
        >
      ...
      {deleteError && (
        <div
          role="alert"
          className="mb-4 flex items-center justify-between rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-700"
        >
      ...
      <div className="flex items-center gap-2 mb-5">
        {FILTERS.map((f) => (
          ...
          >
            ...
            {!loading && f === "All" && (
              <span
                className={`ml-1.5 text-[10px] ${filter === f ? "text-gray-300" : "text-gray-400"}`}
              >
                {data.length}
              </span>
            )}
        ...
        <div className="grid grid-cols-2 gap-3">
          {visibleItems.map((item) => (
            <AnnouncementCard
              ...
              item={item}
              isEditingThis={
                isLoadingDetail && editingId === item.announcement_id
              }
              isPublishingThis={
                isPublishing && publishingId === item.announcement_id
              }
