import { BarChart3, Download, Eye } from "lucide-react";  // ← Eye added
...
export const ReportCard = ({
  ...
  // console.log("ReportCard props:", { title, onPreview, onDownload });
  const colorClasses = {
    ...
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100 hover:border-gray-200">
      ...
      <div className={`bg-gradient-to-r ${colorClasses[color]} p-4 text-white`}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-white/90 truncate mt-0.5">{description}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 bg-gray-50/50">
        <div className="flex justify-between items-center gap-2">
          ...
          {/* Preview — only shown when onPreview is passed ← NEW */}
          {onPreview && (
            <button
              onClick={onPreview}
              className="flex items-center justify-center p-2 rounded-lg bg-white border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all duration-200 group text-indigo-600"
          ...
          {/* Download — only shown when onDownload is passed */}
          {onDownload && (
            <button
              onClick={onDownload}
