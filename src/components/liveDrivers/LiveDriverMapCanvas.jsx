...
function MapToolbar({ map, visibleDrivers }) {
  ...
  useEffect(() => () => { trafficRef.current?.setMap(null); }, []);
  ...
  const clearGeoMarkers = useCallback(() => {
    geoMarkerRef.current?.setMap(null); geoMarkerRef.current = null;
    geoCircleRef.current?.setMap(null); geoCircleRef.current = null;
  }, []);

  const locateMe = useCallback(() => {
    if (!map) return;
    if (geoState === "found") { clearGeoMarkers(); setGeoState("idle"); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords: { latitude: lat, longitude: lng, accuracy } }) => {
        ...
        geoMarkerRef.current = new window.google.maps.Marker({
          ...
          icon: { path: window.google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: "#2563eb", fillOpacity: 1, strokeColor: "#ffffff", strokeWeight: 2.5 },
          zIndex: 999,
        });
        geoCircleRef.current = new window.google.maps.Circle({
          ...
          fillColor: "#2563eb", fillOpacity: 0.08, clickable: false,
        });
        setGeoState("found");
      },
      (err) => {
  ...
  const mapTypeLabel = mapType === "roadmap" ? "🗺️ Map" : mapType === "satellite" ? "🛰️ Sat" : "🌍 Hybrid";
  const geoIcon      = geoState === "locating" ? "⏳" : geoState === "denied" ? "🚫" : "📍";
  const geoLabel     = geoState === "locating" ? "Finding" : geoState === "denied" ? "Denied" : geoState === "found" ? "Clear" : "Find Me";
  ...
  return (
    <div style={TB.wrap}>
      ...
      <button
        title={geoState === "found" ? "Clear my location" : geoState === "locating" ? "Locating…" : geoState === "denied" ? "Location denied" : "Show my location"}
        style={{ ...TB.btn, ...(geoState === "found" ? TB.btnOn : {}), ...(geoState === "denied" ? TB.btnDenied : {}), ...(geoState === "locating" ? TB.btnLoading : {}) }}
        onClick={locateMe}
        disabled={geoState === "locating"}
      >
        <span style={{ ...TB.icon, ...(geoState === "locating" ? { animation: "geo-spin 1s linear infinite", display: "inline-block" } : {}) }}>
          {geoIcon}
        </span>
        <span style={TB.label}>{geoLabel}</span>
        {geoState === "found" && <span style={{ ...TB.dot, background: "#2563eb" }} />}
      </button>

      <button
        title={hasDrivers ? "Fit all visible drivers in view" : "No visible drivers to fit"}
        style={{
          ...TB.btn,
          ...(fitFlash ? TB.btnFit : {}),
...
const TB = {
  ...
  btnLoading: { background: "#f8fafc", border: "1px solid #e2e8f0", cursor: "not-allowed", opacity: 0.7 },
  btnFit:     { background: "#f0fdf4", border: "1px solid #86efac", boxShadow: "0 2px 12px rgba(34,197,94,.25)" },
  btnDisabled:{ opacity: 0.4, cursor: "not-allowed" },
  icon:       { fontSize: 18, lineHeight: 1 },
  label:      { fontSize: 9, fontWeight: 700, color: "#475569", letterSpacing: ".03em", textTransform: "uppercase" },
  dot:        { position: "absolute", top: 6, right: 6, width: 7, height: 7, borderRadius: "50%", background: "#22c55e", border: "1.5px solid #fff" },
};

// ─── MapInner ─────────────────────────────────────────────────────────────────
function MapInner({ driverMap, visibleDrivers, fitKey, selectedDetail, handleMarkerClick, onMapLoad }) {
  const map          = useMap();
  ...
  useEffect(() => {
    ...
    if (pts.length === 1) { map.setCenter(pts[0]); map.setZoom(14); return; }
    const bounds = new window.google.maps.LatLngBounds();
    pts.forEach((p) => bounds.extend(p));
    map.fitBounds(bounds, { top: 80, bottom: 40, left: 280, right: 80 });
  ...
  useEffect(() => {
    ...
    const d = visibleDrivers.find(({ vid, did }) => vid === selectedDetail.vid && did === selectedDetail.did);
  ...
  const driversWithTrails = useMemo(() => {
    ...
    Object.values(posGroups).forEach((group) => {
      if (group.length === 1) { offsetMap.set(`${group[0].vid}/${group[0].did}`, { x: 0, y: 0 }); return; }
      const r = 18;
      group.forEach(({ vid, did }, i) => {
        const a = (i / group.length) * Math.PI * 2;
        offsetMap.set(`${vid}/${did}`, { x: Math.round(Math.cos(a) * r), y: Math.round(Math.sin(a) * r) });
      });
    });
    ...
    visibleDrivers.forEach(({ vid, did, data }) => {
      ...
      result.push({ vid, did, data, trail, offset: offsetMap.get(key) || { x: 0, y: 0 } });
    });

    const active = new Set(visibleDrivers.map(({ vid, did }) => `${vid}/${did}`));
    for (const k of Array.from(trailDataRef.current.keys())) {
      if (!active.has(k)) { trailDataRef.current.delete(k); prevPositionsRef.current.delete(k); }
    }
