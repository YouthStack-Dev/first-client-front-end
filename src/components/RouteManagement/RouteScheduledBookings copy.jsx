...
const RouteScheduledBookings = ({
  ...
  // 🔹 Apply filter: In / Out - Fixed to use stats
  const filteredShiftBookings = shiftsWithBookings.filter((shift) => {
    if (selectedShiftType === "All") return true;
    return selectedShiftType === "In"
      ? shift.log_type === "IN"
      : shift.log_type === "OUT";
  });

  // ✅ Calculate totals from stats instead of bookings array
  const totalLogin = filteredShiftBookings
    .filter((s) => s.log_type === "IN")
    .reduce((sum, s) => {
      const stats = s.stats || { total_bookings: 0 };
      return sum + stats.total_bookings;
    }, 0);
  ...
  // Handler functions for the new columns
  const handleViewUnroutedBookings = (shift) => {
    console.log("View unrouted bookings for shift:", shift.shift_id);
    const stats = shift.stats || {};
    alert(`This shift has ${stats.unrouted_bookings} unrouted bookings`);
    // Navigate to unrouted bookings page or open modal
  };

  const handleViewRoutedBookings = (shift) => {
    console.log("View routed bookings for shift:", shift.shift_id);
    const stats = shift.stats || {};
    alert(`This shift has ${stats.routed_bookings} routed bookings`);
    // Navigate to routed bookings page or open modal
  };

  const handlePendingRouteBookings = (shift) => {
    console.log("Pending bookings for shift:", shift.shift_id);
    alert(`Managing pending bookings for ${shift.shift_code}`);
  };
          ...
          >
            {hasRoutesPermission && (
              <ScheduleList
                ...
                onViewUnroutedBookings={handleViewUnroutedBookings}
                onViewRoutedBookings={handleViewRoutedBookings}
                onPendingRouteBookings={handlePendingRouteBookings}
                isLoading={loading}
