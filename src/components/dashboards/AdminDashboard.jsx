...
const AdminDashboard = () => {
  const user = useSelector((state) => state.user?.user) || { roles: ["ADMIN"], permissions: [] };
  const role = user?.roles?.[0]?.toUpperCase() || ROLES.ADMIN;
  
  console.log("AdminDashboard - User:", user, "Role:", role, "Permissions:", user?.permissions);
