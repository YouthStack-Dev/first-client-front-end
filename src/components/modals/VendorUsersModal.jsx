...
const VendorUserForm = ({
      ...
      if (initialData) {
        ...
        if (initialData.vendor_id && vendors.length > 0) {
          const vendor = vendors.find(
            (v) => String(v.value) === String(initialData.vendor_id)
          );
          if (vendor) {
  ...
  // useEffect 2 — role pre-fill with ref guard, intentionally no dep array
  ...
  useEffect(() => {
    if (
      ...
      const role = roleOptions.find(
        (r) => String(r.value) === String(initialData.role_id)
      );
      if (role) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Invalidate Redux cache so the list refetches fresh data on next render.
      // Sets lastFetched = null in the slice → fetchIfNeeded in VendorUserManagement
      // sees stale data and triggers a fresh fetch automatically via handleVendorUserSuccess.
      dispatch(invalidateVendorUsers());
