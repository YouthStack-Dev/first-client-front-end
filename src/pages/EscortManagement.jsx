import React, { useEffect, useState, useMemo } from "react"; // removed useRef
import { useDispatch, useSelector } from "react-redux";
import { X, Eye, EyeOff, Trash2, Truck, Search } from "lucide-react"; // added Search
...
const EscortManagement = () => {
  ...
  const loaded           = useSelector(selectEscortLoaded);
  const lastVendorFilter = useSelector(selectEscortLastVendor);
  ...
  useEffect(() => {
    const currentFilter = selectedVendorId || null;
    const filterChanged = loaded && lastVendorFilter !== currentFilter;
    const notYetLoaded  = !loaded;

    if (notYetLoaded || filterChanged) {
      dispatch(
        fetchEscortsThunk(selectedVendorId ? { vendor_id: selectedVendorId } : {})
      );
    }
  }, [dispatch, selectedVendorId, loaded, lastVendorFilter]);

  // ── Client-side search filter ─────────────────────────────────────────────
  const filteredEscorts = useMemo(() => {
    if (!searchQuery.trim()) return escorts;
    const q = searchQuery.toLowerCase();
    return escorts.filter(
      (e) =>
  ...
  const handleOnToggleActive = async (escort) => {
    try {
      await dispatch(toggleEscortActiveThunk({ id: escort.escort_id, currentState: escort.is_active })).unwrap();
      toast.success(`Escort ${escort.is_active ? "deactivated" : "activated"} successfully!`);
    } catch (error) {
  ...
  const handleOnToggleAvailable = async (escort) => {
    try {
      await dispatch(toggleEscortAvailableThunk({ id: escort.escort_id, currentState: escort.is_available })).unwrap();
      toast.success(`Escort marked as ${escort.is_available ? "unavailable" : "available"}!`);
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    else if (phoneDigits.length < 10) newErrors.phone   = "Phone must have at least 10 digits";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (modalMode === "create") {
        ...
        if (formData.password?.trim()) createPayload.password = formData.password.trim();
        await dispatch(createEscortThunk(createPayload)).unwrap();
        toast.success("Escort created successfully!");
      } else {
        await dispatch(updateEscortThunk({ id: selectedEscort.escort_id, data: basePayload })).unwrap();
  ...
  const submitResetPassword = async () => {
    ...
    try {
      await dispatch(setEscortPasswordThunk({ escortId: passwordEscort.escort_id, newPassword: value })).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        searchBar={
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-8 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all w-full min-w-[220px]"
            />
            {searchQuery && (
              <button
        ...
        rightElements={
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 rounded-lg border border-gray-200">
              ...
              <span className="text-sm font-bold text-gray-800">{filteredEscorts.length}</span>
            </div>
      ...
      {/* Table — pass filteredEscorts instead of escorts */}
      <EscortTable
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md overflow-hidden rounded-lg border border-app-border bg-app-surface shadow-xl">
            <div className="flex items-center justify-between bg-gradient-to-r from-sidebar-primary to-sidebar-secondary px-6 py-4">
              ...
              <button type="button" onClick={closeResetPasswordModal} disabled={isResettingPassword}
            ...
            <div className="space-y-4 px-6 py-5">
              <p className="text-sm text-app-text-secondary">
                ...
                <span className="font-semibold text-app-text-primary">{passwordEscort?.name || "selected escort"}</span>.
              </p>
              {["New Password", "Confirm Password"].map((lbl, i) => (
                <div key={lbl}>
                  <label className="mb-1 block text-sm font-medium text-app-text-secondary">{lbl}</label>
                  <div className="relative">
                    <input
                      type={showResetPassword ? "text" : "password"}
                      value={i === 0 ? resetPassword : confirmResetPassword}
                      onChange={(e) => i === 0 ? setResetPassword(e.target.value) : setConfirmResetPassword(e.target.value)}
                      ...
                      placeholder={i === 0 ? "Enter new password" : "Re-enter new password"}
                    />
                    <button type="button" onClick={() => setShowResetPassword((v) => !v)} disabled={isResettingPassword}
            ...
            <div className="flex justify-end gap-3 border-t border-app-border bg-app-tertiary px-6 py-4">
              ...
              <button type="button" onClick={submitResetPassword} disabled={isResettingPassword}
      ...
      {deleteConfirm.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl border border-gray-200 shadow-lg p-6 w-80 text-center">
            ...
            <p className="font-semibold text-gray-800 text-base mb-1">Delete escort?</p>
            <p className="text-sm text-gray-500 mb-5">This action cannot be undone.</p>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setDeleteConfirm({ open: false, escortId: null })}
