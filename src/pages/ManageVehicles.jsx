...
//             .map((e, i) => e.msg || JSON.stringify(e))
//             .join(", ");
//         } else {
...
//       dispatch(fetchVehicleTypes(tenantId)).unwrap().catch(() => {
...
//     // Fetch vehicles on vendor/status/page changes
// useEffect(() => {
//   if (!selectedVendor) {
...
//   console.log("[fetchVehicles] Dispatching fetch with params:", {
...
//     .unwrap()
//     .then((res) => {
//       console.log("[fetchVehicles] Success — vehicles fetched:", res);
//     })
//     .catch((err) => {
//       console.error("[fetchVehicles] Failed to load vehicles:", err);
//       toast.error("Failed to load vehicles");
//     });
// }, [selectedVendor, viewActive, offset, itemsPerPage, dispatch]);

//   // Filter vehicles locally by reg_number search term (case insensitive)
...
//       if (
//         window.confirm(
//           `Are you sure you want to delete vehicle ${vehicle.vehicle_code} (${vehicle.reg_number})?`
//         )
//       ) {
//         dispatch(
//           deleteVehicleThunk({
...
//               const name = vendor.vendor_name || `Vendor ${id}`;
//               return (
...
function ManageVehicles() {
  ...
  }, [tenantId, vendors.length, vendorLoading, dispatch]);

  // Fetch vehicle types
  useEffect(() => {
    if (tenantId && vehicleTypes.length === 0 && !vehicleTypeLoading) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="w-full md:w-1/3">
          ...
          >
            <option value="">Select Vendor</option>
            {vendors.map((vendor) => (
              <option key={vendor.id || vendor.vendor_id} value={String(vendor.id || vendor.vendor_id)}>
