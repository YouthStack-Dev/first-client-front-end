...
const VehicleList = React.memo(({ vehicles, onNext, onPrev, currentPage, totalPages, isLoading, error, handleEdit, handleDelete }) => {
  ...
  return (
    <div className="rounded-lg border bg-white shadow-sm mt-4">
      ...
      <div className="flex justify-center items-center gap-4 py-4">
        <button onClick={onPrev} disabled={currentPage === 1} className={`flex items-center gap-2 px-4 py-2 rounded ${currentPage === 1 ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-gray-200 text-gray-800 hover:bg-gray-300"}`}>
          <ChevronLeft size={18} /> Prev
        </button>
        <span className="text-sm text-gray-700 font-medium">Page {currentPage} of {totalPages}</span>
        <button onClick={onNext} disabled={currentPage === totalPages} className={`flex items-center gap-2 px-4 py-2 rounded ${currentPage === totalPages ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700"}`}>
...
function ManageVehicles() {
  ...
  const tenantId = useSelector((state) => state.auth.user?.tenant_id);
  const { vendors, vendorLoading, vehicleTypes, vehicleTypeLoading } = useSelector((state) => ({
    vendors: state.vendor.vendors,
    vendorLoading: state.vendor.loading,
    vehicleTypes: state.vehicleType.vehicleTypes,
    vehicleTypeLoading: state.vehicleType.loading,
  ...
  useEffect(() => {
    if (selectedVendor) {
      dispatch(fetchVehicles({ vendorId: selectedVendor, status: viewActive ? "active" : "inactive", offset: 0, limit: 10 }));
    }
  }, [selectedVendor, viewActive, dispatch]);

  const filteredVehicles = vehicles.filter((v) => v.reg_number?.toLowerCase().includes(searchTerm.toLowerCase()));

  const onPrev = () => currentPage > 1 && setCurrentPage((prev) => prev - 1);
  const onNext = () => currentPage < totalPages && setCurrentPage((prev) => prev + 1);
