  ...
  reducers: {
    ...
    removeTeam(state, action) {
      ...
      if (removeEmployees) {
        ...
        employeeIds.forEach((id) => {
          ...
          state.employees.allIds = state.employees.allIds.filter(
            (eId) => eId !== id
          );
        });
      }
    ...
    addEmployeeToTeam(state, action) {
      ...
      if (
        ...
        const currentCount =
          state.teams.byId[teamId].active_employee_count || 0;
    ...
    setDepartmentEmployees(state, action) {
      ...
      logDebug(
        `Cached ${employeeCodes.length} ${
          isActive ? "active" : "inactive"
        } employees for department ${departmentId}`
      );
    },
    ...
    moveEmployee(state, action) {
      ...
      if (state.teams.byId[fromTeamId]) {
        state.teams.byId[fromTeamId].employeeIds = state.teams.byId[
          fromTeamId
        ].employeeIds.filter((id) => id !== employee_id);
      }

      if (
        state.teams.byId[toTeamId] &&
    ...
    moveEmployeeToInactive: (state, action) => {
      ...
      if (state.teams.byId[departmentId]) {
        ...
        team.active_employee_count = Math.max(
          0,
          (team.active_employee_count || 0) - 1
        );
    ...
    moveEmployeeToActive: (state, action) => {
      ...
      if (state.teams.byId[departmentId]) {
        ...
        team.inactive_employee_count = Math.max(
          0,
          (team.inactive_employee_count || 0) - 1
        );
      }
    },
    ...
    updateEmployeeStatus: (state, action) => {
      const { employeeId, isActive, departmentId, employeeData } =
        action.payload;
      ...
      if (state.departmentEmployees[departmentId]) {
        ...
        if (department.active) {
          department.active = department.active.filter(
            (id) => id !== employeeId
          );
        }
        if (department.inactive) {
          department.inactive = department.inactive.filter(
            (id) => id !== employeeId
          );
        }
